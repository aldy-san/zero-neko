                    ...
                    <span className="space-x-2">
                        {
                            props.datas.variants[0].written.split("").map((kana, idx) => {
                                return <Link key={idx} to={"/kanji/"+kana} className={isKanji(kana) ? "transition-all duration-150 border-dashed border-b-2 border-primary hover:text-5xl hover:text-primary" : ""}>{kana}</Link>;
                            })
                        }
                    </span>
                </p>
