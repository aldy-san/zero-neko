...
const FallingWords = () => {
    ...
    const difficulties = [
        {
            ...
            name: 'Beginner',
            speed: 3,
            words: all.filter(word => word.expression.length < 4)
        },{
            id: 1,
            name: 'Easy',
            speed: 8,
            ...
            name: 'Medium',
            speed: 12,
            words: all.filter(word => word.expression.length < 5 && word.expression.length > 3)
        },{
            id: 3,
            name: 'Hard',
            speed: 16,
            words: all.filter(word => word.expression.length > 3)
        }
    ]

    const [game, setGame] = useState(new Game(difficulties[selectedDifficulty]))
    ...
    function loadGame(difficultyId) {
        ...
        let game = new Game(difficulties[difficultyId])
    ...
    function handleInput(e) {
        ...
        let correctWord = words.findIndex(word => word.text === value.trim())
        if (correctWord > -1) {
            setCurrentText('')
...
class Game {
    constructor(difficulty) {
        ...
        this.words = difficulty.words.map(word => { return new Word(word, this.width) })
    }
    getRandomWord() {
